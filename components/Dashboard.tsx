...
export default function DashboardComponent() {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [createdRes, toApproveRes, claimedRes] = await Promise.all([
          axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/created_bounties`),
          axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user/bounties-to-approve`),
          axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user/claimed-bounties`)
        ]);
    ...
    const handleDeleteBounty = async (bountyId: number) => {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/bounty/${bountyId}`);
        // After successful deletion, refresh the created bounties list
        setCreatedBounties(prevBounties => prevBounties.filter(bounty => bounty.id !== bountyId));
      } catch (error) {
          ...
          {createdBounties.length > 0 ? (
            createdBounties.map(bounty => (
              <div key={bounty.id}>
                ...
                <a href={bounty.issue_url} target="_blank" rel="noopener noreferrer">View Issue</a>
                <button onClick={() => handleDeleteBounty(bounty.id)}>Delete Bounty</button>
              </div>
            ))
          ) : (
            <div>
          ...
          <div key={bounty.id}>
            ...
            {bounty.claimants.map(claimant => (
              <div key={claimant.claimant_id}>
                <p>{claimant.claimant_name} ({claimant.claimant_email})</p>
                <button onClick={() => handleApproveBounty(bounty.id, claimant.claimant_id)}>
